...
class RIBCorpus:
    """Streams AS paths from a pipe-separated RIB file without loading all routes into memory.

    Each line is expected in the format: prefix|AS1|AS2|...|ASn
    The first line (header) is skipped automatically.
    """

    def __init__(self, rib_path, skip_header=True):
        self.rib_path = rib_path
        self.skip_header = skip_header

    def __iter__(self):
        with open(self.rib_path, 'r') as f:
...
class BGP2VEC:
    def __init__(self, model_path, bgp_routes=None, rewrite=False, embedding_size=32, negative=5, epochs=3, window=2,
        ...
        if rewrite or not self.import_model():
            logging.info(("Start generating BGP2VEC model for ", bgp_routes))
    ...
    def build_model(self, embedding_size, window, negative, epochs):
        ...
        # corpus_count is set by build_vocab — works with lists and lazy iterables alike
        self.model.train(self.routes, total_examples=self.model.corpus_count, epochs=epochs, report_delay=30)
