...
def generate_routes_from_file_handler(f, remove_dup=True, remove_first=False, by_vantage=False, by_ap=False, by_asn=False,
    ...
    print("Generating routes from file handler ...")
    counter = 0
    if remove_first:
        start_ind = 7
    else:
    ...
    if by_vantage:
        ...
        print("No by_vantage, by_ap, by_asn")
        routes = []
        for i, line in enumerate(f):
            # print("[counter]",counter)
            line = line.split() # ['*', '1.23.167.0/24', '62.115.128.137', '0', '0', '0', '1299', '1273', '55410', '45528', 'i']
            
            if len(line) > 0 and line[0] == '*':
                
                #Si el path contiene {} o () se ignora path
                # print("Linea:",line)
                if any('{' in field or '(' in field for field in line):
                    continue  # Omite esta ruta si contiene '{' o '('
