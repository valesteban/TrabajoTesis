...
class MLPPredictorEmbeddings(nn.Module):
    ...
    def apply_edges(self, edges):
        ...
        """
        h_src = edges.src['h']  # Obtener los embeddings del nodo fuente
        h_dst = edges.dst['h']
        h = torch.cat([h_src, h_dst], 1)
        return {'score': self.W2(F.relu(self.W1(h))).squeeze(1)}

    def forward(self, g, emb):
        with g.local_scope():
            # Obtener los embeddings para los nodos dados
            g.ndata['h'] = emb
            g.apply_edges(self.apply_edges)
            return g.edata['score']
